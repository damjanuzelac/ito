//! Audio preparation: light enhancement of 16-bit PCM, WAV encoding for the
//! upload, a loudness measure used to reject silence, and a minimal WAV
//! reader for `--transcribe-file`.
//!
//! The enhancement is ported from the Ito server (server/src/utils/audio.ts).

pub const SAMPLE_RATE: u32 = 16000;

/// Root-mean-square amplitude, in the same 0..32767 scale as the samples.
///
/// Used as a speech gate: a clip below the configured floor is never sent to
/// the API, which both saves a request and avoids the transcriber inventing a
/// sentence out of room tone.
...
/// Wraps 16 kHz mono samples in a WAV container ready for upload.
pub fn to_wav(samples: &[i16]) -> Vec<u8> {
    let data_length = (samples.len() * 2) as u32;
    let mut wav = Vec::with_capacity(44 + samples.len() * 2);
    wav.extend_from_slice(&create_wav_header(data_length, SAMPLE_RATE, 1, 16));
...
mod tests {
    ...
    fn wav_roundtrip() {
        let samples: Vec<i16> = (0..1600)
            ...
            .collect();
        let (parsed, rate) = read_wav_pcm16(&to_wav(&samples)).unwrap();
    ...
    fn rms_separates_silence_from_speech() {
        assert_eq!(rms(&[]), 0.0);
        assert_eq!(rms(&[0; 16000]), 0.0);

        // Room tone: a handful of LSBs of noise stays far below the default
        // floor of 200, while a normal-level tone sits far above it.
        let quiet: Vec<i16> = (0..16000).map(|i| ((i % 7) as i16) - 3).collect();
        assert!(rms(&quiet) < 200.0, "room tone measured {}", rms(&quiet));

        let speech: Vec<i16> = (0..16000)
            .map(|i| {
                let t = i as f32 / 16000.0;
                ((t * 200.0 * 2.0 * std::f32::consts::PI).sin() * 6000.0) as i16
            })
            .collect();
        assert!(rms(&speech) > 200.0, "speech measured {}", rms(&speech));
    }
